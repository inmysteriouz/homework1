def get_mask_card_number(card_number: str | None) -> str:
    ...
    Остальные цифры заменяются на символы '*' и ' '."""
    if not card_number:
        return ""
...
def get_mask_account(account_number: str | None) -> str:
    ...
    Маскирует номер счета, оставляя видимыми только последние 2 цифры, если длина номера меньше 4 символов.
    В противном случае, все цифры заменяются на '*' кроме последних 4 символов."""
